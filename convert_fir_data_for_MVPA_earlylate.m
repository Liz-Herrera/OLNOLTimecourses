...
% old (from convert_fir_data_for_MVPA script): (OL_new, NOL_new, OL_old, NOL_old)
...
%in this section, the variables (e.g., indiv_mean_new_ol) are referring to
%the variables loaded in from the "indiv_timecourses.mat" file (or, the
%"early-late" version of that file)
...
for i = 1:length(indiv_mean_new_ol{1}) %we arbitrarily look at the first \\
    ...
    % <-this variable name is again from indiv_timecourses.mat (or
    % early-late)
...
s = length(indiv_mean_new_ol); % multiplier that counts number of subjects temp_ol_new_early_names
...
testmat = [temp_ol_new_early_names_mat temp_ol_new_late_names_mat temp_nol_new_early_names_mat temp_nol_new_late_names_mat];
